...
def get_mol_wt_reward(base_mol_wt, gen_token, vocab_list):
    gen_seq_list = ["".join([vocab_list[idx] for idx in row]) for row in gen_token]
    mol_wt = np.array([molecular_weight(seq, seq_type='protein') for seq in gen_seq_list])
    return (base_mol_wt - mol_wt)/base_mol_wt
...
def loss_func(f_type="pre_worker"):
    ...
    if f_type == "pre_worker":
        ...
        def func(rewards, real_goal, delta_feature, gen_token, vocab_list, base_mol_wt=3000, mol_wt_reward_mul=1.0):
            mol_wt_reward = get_mol_wt_reward(base_mol_wt, gen_token, vocab_list) # get reward for mol wt less than base mol wt

            rewards_combined = torch.from_numpy(np.array([[max(0, (r + mol_wt_reward_mul * mol_wt_reward[i]).item()) for r in row] for i, row in enumerate(rewards)])) #combined reward
            
            loss = -torch.mean(rewards_combined*(1.0 - F.cosine_similarity(delta_feature, real_goal, dim=2)))
            return loss
        return func
    elif f_type == "dis":
