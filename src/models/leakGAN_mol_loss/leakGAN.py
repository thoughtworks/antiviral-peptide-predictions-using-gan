...
parser.add_argument("--data_path", type=str, default="/save_files/", metavar="PATH",  ### check this again
...
parser.add_argument("--vocab_size", type=int, default=10, metavar="N",  ####changed to 10
                    help="Vocabulary size (defaul: 10)")
parser.add_argument("--batch_size", type=int, default=10, metavar="N",  ####changed this  from 64 to 10
                    help="Batch size(defaul: 10)")
parser.add_argument("--n_samples", type=int, default=6400, metavar="N",  ###check this again not so sure
                    help="Number of samples generated per time(defaul: 6400)")  ## what value to put for our case
...
g_embed_dim = 32
g_hidden_dim = 32
g_seq_len = 20  ## by default
#   MANAGER:
g_m_batch_size = 10  ##changed from 64 to 10
g_m_hidden_dim = 32
g_m_goal_out_size = 0
#   WORKER:
g_w_batch_size = 10  ##changed from 64 to 10
g_w_vocab_size = 20
g_w_embed_dim = 32
g_w_hidden_dim = 32
g_w_goal_out_size = 0
g_w_goal_size = 16  ##doubt on this parameter
g_step_size = 5
...
def get_arguments():
    train_params = get_params("params/train_params.json")  ##changed for our data
...
# List of models
def prepare_model_dict(use_cuda=True):  ##changed this to true
    ...
    )  ##goal out size assigned to num filter _num_filters = [100,200,200,200,200,100,100,100,100,100,160,160],
...
def prepare_optimizer_dict(model_dict, lr_dict):  # lr_dict = learning rate
    ...
    m_lr = lr_dict["manager"]  ##generator learning rate
    w_lr = lr_dict["worker"]
    d_lr = lr_dict["discriminator"]

    w_optimizer = optim.Adam(worker.parameters(), lr=w_lr)  ###adam optimizer
...
def pretrain_generator(model_dict, optimizer_dict, scheduler_dict, dataloader, vocab_size, max_norm=5.0, use_cuda=False,
    ...
    for i, sample in enumerate(dataloader):  ##done changes in the loss
        ...
        # Calculate pretrain loss      ###in the if condition changes are done
        if (sample.size() == torch.zeros(
                [10, 20]).size()):  # sometimes smaller than 64 (16) is passed, so this if statement disables it
            # print("Sample size: {}".format(sample.size()))   ##changed zeros matrix from 64,20 to 10,20 ##do the change
...
def generate_samples(model_dict, negative_file, batch_size,
    ...
    neg_data = []
    # getting the argument for saving
    ...
    #
    #
    # ('negdata_linumber241:', neg_data)
    np.save(negative_file, neg_data)
    ### code to save the last generated seq
    for epoch in range(param_dict["train_params"]["total_epoch"]):
        if epoch == range(param_dict["train_params"]["total_epoch"])[-1]:
            save('last_generated_sample', neg_data)
            np.savetxt('last_generated_sample.txt', neg_data)
            # print(neg_data)
...
def pretrain_discriminator(model_dict, optimizer_dict, scheduler_dict,
    ...
    # here the dataset is being loaded (bit uncertain)
    ...
    # print('dis_dataloader_params_line_256',dis_dataloader_params)
    # print(positive_file,negative_file)  ## to check
    ...
    for epoch in range(epochs):
        for i, sample in enumerate(dataloader):
            ...
            d_optimizer.step()
            if i == 9:  ##batch size 10 using enumerate without start so indexing it from 0
...
def adversarial_train(model_dict, optimizer_dict, scheduler_dict, dis_dataloader_params,
                      vocab_size, vocab_list, pos_file, neg_file, batch_size, gen_train_num=80,  ### update this after training
                      dis_train_epoch=100, dis_train_num=30, max_norm=5.0,  ##updates are always bounded.
    ...
    for _ in range(gen_train_num):
        ...
        m_loss = loss_func("adv_manager")(rewards, real_goal, delta_feature, gen_token, vocab_list, base_mol_wt=3000, mol_wt_reward_mul=1.0)
        w_loss = loss_func("adv_worker")(all_goal, delta_feature_for_worker, gen_token, prediction, vocab_size,
                                         use_cuda)

        torch.autograd.grad(m_loss, manager.parameters())  # based on loss improve the parameters
        torch.autograd.grad(w_loss, worker.parameters())
        clip_grad_norm_(manager.parameters(), max_norm)  ##use of max_norm = 5
        clip_grad_norm_(worker.parameters(), max_norm)
        m_optimizer.step()
        w_optimizer.step()
        print("Adv-Manager loss: {:.5f} Adv-Worker loss: {:.5f}".format(m_loss, w_loss))
        # print(real_goal) ## remove this
        # why the deletion???
    del adv_rets
    del real_goal
    del all_goal
    ...
    for n in range(dis_train_epoch):
        ...
        # print(outs) ### check what is this printing
...
def save_checkpoint(model_dict, optimizer_dict, scheduler_dict, ckpt_num, replace=False):
    ...
    torch.save({"model_dict": model_dict, "optimizer_dict": optimizer_dict, "scheduler_dict": scheduler_dict,
                "ckpt_num": ckpt_num}, file_name)
    if replace:
...
def main():
    ...
    vocab_list = pickle.load(open(vocab_file,'rb'))
    ...
    batch_size = param_dict["train_params"]["generated_num"]  ## previous generated_num = 156 changed to 10 (check!)
    ...
    for i in range(param_dict["train_params"]["pre_dis_epoch_num"]):
        ...
        model_dict, optimizer_dict, scheduler_dict = pretrain_discriminator(model_dict, optimizer_dict, scheduler_dict,
                                                                            dis_data_params, vocab_size=vocab_size,
    ...
    for epoch in range(param_dict["train_params"]["pre_gen_epoch_num"]):
        ...
        model_dict, optimizer_dict, scheduler_dict = pretrain_generator(model_dict, optimizer_dict, scheduler_dict,
    ...
    vocab_size = param_dict["leak_gan_params"]["discriminator_params"]["vocab_size"]
    save_num = param_dict["train_params"][
        "save_num"]  # save checkpoint after this number of repetitions changed from 10 to 5
    replace_num = param_dict["train_params"]["replace_num"]

    for epoch in range(param_dict["train_params"]["total_epoch"]):
        print("Epoch: {}/{}  Adv".format(epoch, param_dict["train_params"]["total_epoch"]))
        model_dict, optimizer_dict, scheduler_dict = adversarial_train(model_dict, optimizer_dict, scheduler_dict,
                                                                       dis_data_params, vocab_size=vocab_size,
                                                                       vocab_list=vocab_list,
                                                                       pos_file=pos_file, neg_file=neg_file,
                                                                       batch_size=batch_size, use_cuda=use_cuda,
                                                                       epoch=epoch,
                                                                       tot_epoch=param_dict["train_params"][
                                                                           "total_epoch"])
        if (epoch + 1) % save_num == 0:
            ckpt_num += 1
            if ckpt_num % replace_num == 0:
                save_checkpoint(model_dict, optimizer_dict, scheduler_dict, ckpt_num,
                                replace=True)  ## to get individual checkpoint files
            else:
                save_checkpoint(model_dict, optimizer_dict, scheduler_dict, ckpt_num)
